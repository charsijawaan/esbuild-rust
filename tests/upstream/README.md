...
The active bundler tranche exact-compares 505 Unix snapshots from the default,
...
`KeepNames`, and `MinifySyntax`. This includes dynamic `require()` and
`import()` glob modules in both single-file and code-splitting builds, glob
import attributes, empty-glob warnings, and entry-point glob expansion. The
missing-glob-directory diagnostic-only case is also covered. So 4,797 concrete
upstream cases are currently active in
`cargo test`; the remaining captured bundler cases are the parity backlog, not
